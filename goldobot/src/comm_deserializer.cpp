...
size_t CommDeserializer::push_data(unsigned char* buffer, size_t buffer_size) {
  ...
  m_statistics.buffer_high_watermark =
      std::max<uint32_t>(m_statistics.buffer_high_watermark, size());
  return bytes_received;
}

size_t CommDeserializer::size() const {
  return m_end_index >= m_begin_index ? m_end_index - m_begin_index
                                      : m_end_index - m_begin_index + m_buffer_size;
}

bool CommDeserializer::message_ready() const { return m_state == MessageReady; }

uint16_t CommDeserializer::message_type() const { return m_message_type; }
size_t CommDeserializer::message_size() const { return m_message_size; }
