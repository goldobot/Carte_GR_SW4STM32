...
CommSerializer::CommSerializer(unsigned char* buffer, size_t size)
    : m_buffer(buffer), m_buffer_size(size), m_begin_index(0), m_end_index(0) {}

bool CommSerializer::push_message(uint16_t message_type, const unsigned char* buffer,
                                  size_t msg_size) {
...
size_t CommSerializer::availableSize() const { return m_buffer_size - (size() + 12); }
