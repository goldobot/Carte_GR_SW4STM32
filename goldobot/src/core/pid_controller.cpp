...
void PIDController::set_config(const PIDConfig& config) { m_config = config; }

void PIDController::set_kp(float kp) { m_config.kp = kp; }

void PIDController::set_kd(float kd) { m_config.kd = kd; }

void PIDController::set_ki(float ki) { m_config.ki = ki; }
...
float PIDController::update(float current_value) {
  ...
  if (!m_first_run) {
    derivative_term =
        (m_target_derivative - (current_value - m_previous_value) / m_config.period) * m_config.kd;
