...
ArmsTask::ArmsTask() : m_message_queue(m_message_queue_buffer, sizeof(m_message_queue_buffer)) {}

const char* ArmsTask::name() const { return "arms"; }
...
void ArmsTask::process_message() {
  ...
  switch (m_message_queue.message_type()) {
    case CommMessageType::DbgDynamixelsList: {
      ...
      for (unsigned id = 0; id < 0xFE; id++) {
        if (dynamixels_read_data(id, 0, buff, 4)) {
          Robot::instance().mainExchangeOut().pushMessage(CommMessageType::DbgDynamixelDescr,
    ...
    case CommMessageType::DbgArmsSetPose: {
      ...
      m_message_queue.pop_message((unsigned char*)buff,
                                  (m_config.num_servos + 1) * sizeof(uint16_t));
