...
const char* UART2CommTask::name() const
{
	return "uart_comm";
}

void UART2CommTask::init()
{
	Robot::instance().mainExchangeOut().subscribe({2000,3000, &m_out_queue});
	Task::init();
}

void UART2CommTask::taskFunction()
{
	set_priority(5);
	...
	Hal::uart_receive(2, m_recv_buffer, sizeof(m_recv_buffer), false);
	while(1)
	{
		// If current transmission is finished, send next chunk of data from ring buffer
		if(Hal::uart_transmit_finished(2))
		{
			...
			if(dtlen)
			{
				Hal::uart_transmit(2, m_send_buffer, dtlen, false);
			}
			m_bytes_sent += dtlen;
		}

		// Parse received data
		if(Hal::uart_receive_finished(2))
		{
			m_deserializer.push_data((unsigned char*)m_recv_buffer, sizeof(m_recv_buffer));
		} else
		{
			// Abort reception if previous was not finished
			uint16_t bytes_received = Hal::uart_receive_abort(2);
			if(bytes_received)
			{
...
bool UART2CommTask::send_message(CommMessageType type, const char* buffer, uint16_t size)
{
	m_serializer.push_message((uint16_t)type, (const unsigned char*)(buffer), size);
	return true;
}

void UART2CommTask::process_message(uint16_t message_type)
{
