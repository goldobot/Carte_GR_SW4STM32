...
namespace goldobot
{
	...
	public:
		...
		//! \brief Push new command at the back of the queue, return true on success, false if queue was full.
		bool push(const PropulsionCommand&& command);

		//! \brief Pop current command from the queue and return pointer to new current command, or nullptr if the queue is empty
